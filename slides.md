...
// 混算需手动转：1n + BigInt(1) → 2n；Number(1n) + 1 → 2（大值丢精度）
...
speed: -1 → -0.5 → **-0**（负方向减速至停，-0 保留了"负方向"）

speed:  1 →  0.5 →  **0** （正方向减速至停）
...
**注意：** 赋值传递的是对象的内存地址，不是变量绑定。a 和 b 各自存了一份地址，指向同一对象。`b = \[4,5,6\]` 改的是 b 存的地址，不影响 a。
...
**函数参数传递的是对象地址的副本，不是变量绑定。** x 和 a 各自存了一份地址，指向同一数组。
